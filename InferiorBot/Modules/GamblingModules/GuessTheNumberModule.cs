...
namespace InferiorBot.Modules.GamblingModules
{
    ...
    public class GuessTheNumberModule(InferiorBotContext context, IServiceProvider services) : BaseUserModule(context, services)
    {
        ...
        public class StartGameModules(InferiorBotContext context, IServiceProvider services) : BaseUserModule(context, services)
        {
            ...
            public async Task HandleSlashCommand([Summary(description: "How much money is required to play?")] string? amount = null)
            {
        ...
        public async Task HandleSlashCommand([Summary(description: "Enter a number between 0 and 100."), MinValue(0), MaxValue(100)] int number)
        {
            var gameType = await _context.GameTypes.FirstOrDefaultAsync(x => x.Name == "guess the number");
            if (gameType == null)
            {
